# PRD: One Candidate Runs As Bounded Live Trader-System Pod
...
One promoted `TraderSystemCandidate` runs as a `TradingSystemPod` under live binding on Binance BTC
perpetual futures.

## In-Scope Behavior

- create a live `StageBinding` for one promoted candidate
- launch one live `TradingSystemPod`
- resolve one `AgentRuntimeUnit` with `runtime_unit_role=live_operator_agent` by default
- use `AgentLoopPolicy` with `continuous_live`
- expose live market data and account state through `ToolProxy`
- allow the agent brain to produce `OrderIntent`
- route order intent through autokairos risk/execution gateway as a durable `GatewayDecision`
- record `ExecutionAttempt`
- keep image/package/evidence/promotion links inspectable
...
- wake/intervention as the main contract
- full A2A mesh or distributed multi-pod coordination
...
- live pod is not the same evaluated artifact
...
- live state links back to candidate, image, packages, evidence, and promotion
- no direct unrestricted exchange access is available to the agent harness

## Metrics / Proof

- live pod produces meaningful bounded trading behavior
...
- agent-system: brain session and pod runtime behavior
