...
- the stable runtime-connector interface
...
```mermaid
flowchart LR
    A["Runtime connector interface"] --> B["Execution driver"]
    B --> C["Runtime / harness"]
```
...
autokairos governs an already-existing harness through the runtime connector.
