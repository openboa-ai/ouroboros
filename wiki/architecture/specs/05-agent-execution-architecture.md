...
external runtime sessions. Multica separates its daemon/runtime bridge from higher-level runtime,
...
- an external runtime bridge path
...
```mermaid
flowchart LR
    A["Control plane"] --> B["Runtime bridge"]
...
- the runtime bridge turns that decision into a live runtime session
...
The runtime bridge is the layer that turns a governed run request into a live agent loop.
...
### Runtime bridge non-responsibilities
...
Without a distinct runtime bridge, autokairos would be forced into one of two bad designs:
...
Instead, the runtime bridge should emit external trace data as execution proceeds:
...
- the runtime-bridge method signature document
...
The runtime bridge starts or wakes the agent loop.
...
The runtime bridge and stage binding layers are what make that possible without asking the prompt
