...
export interface CandidateRunRecord {
  ...
  run_id: string;
  candidate_id: string;
  candidate_ref: { record_kind: "trader_system_candidate"; id: string };
...
export async function runPromotedCandidate(
  ...
  const record: CandidateRunRecord = {
    record_kind: "trader_system_candidate_run",
    version: 1,
    run_id: runId,
    candidate_id: options.candidateId,
    candidate_ref: { record_kind: "trader_system_candidate", id: options.candidateId },
...
function printHelp(): void {
  console.log(`Usage: npm run trading:candidate:run -- --candidate-id <id>
       npm run trading:candidate:run -- --candidate-id <id> --runner docker_sandboxes_sbx

Runs a promoted TraderSystemCandidate by candidate id through replay scenarios.
...
function printRunResult(record: CandidateRunRecord): void {
  console.log("Trading candidate run");
...
function defaultRunId(candidateId: string, date: Date): string {
  return `candidate-run-${safeId(candidateId)}-${timestampId(date)}`;
}
...
if (executedPath === currentModulePath) {
  runCandidateFromCli().catch((error: unknown) => {
