...
export type BtcPerpFixtureEvaluationScenario =
  | "accepted_oos_survives_costs"
  | "quarantined_metric_instability"
  | "disqualified_lookahead_leakage";

export interface BtcPerpAarEvaluationSubmission {
...
export interface DeterministicBtcPerpSealedEvaluatorOptions {
  evaluator_ref?: Ref;
}

export class DeterministicBtcPerpSealedEvaluator {
  private readonly evaluatorRef: Ref;

  constructor(options: DeterministicBtcPerpSealedEvaluatorOptions = {}) {
  ...
  evaluate(submission: BtcPerpAarEvaluationSubmission): TradingEvaluationResultRecord {
    ...
    const base = {
      ...
      version: 1 as const,
      trading_evaluation_result_id: `trading-evaluation-result-${safeId(submission.experiment.aar_experiment_id)}`,
      aar_experiment_ref: ref("aar_experiment", submission.experiment.aar_experiment_id),
      ...
      metric_refs: submission.metric_refs ?? metricRefsFor(submission.experiment.aar_experiment_id, scenario),
      evaluator_trace_ref: ref(
        "trace_placeholder",
        `trace-sealed-btc-perp-${safeId(submission.experiment.aar_experiment_id)}`
      ),
  ...
  private assertSubmission(submission: BtcPerpAarEvaluationSubmission): void {
    if (submission.task.market_scope !== "binance_btc_perpetual_futures") {
...
function scenarioFromArtifact(runnableArtifactId: string): BtcPerpFixtureEvaluationScenario {
...
function metricRefsFor(experimentId: string, scenario: BtcPerpFixtureEvaluationScenario): Ref[] {
