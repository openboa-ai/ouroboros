...
describe("S18 Trading candidate run scripts", () => {
  it("runs a promoted candidate by candidate id and records candidate-run evidence", async () => {
    const tempDir = await mkdtemp(path.join(tmpdir(), "ouroboros-s18-candidate-run-"));
    try {
      ...
      expect(result.stdout).toContain("Trading candidate run");
  ...
  it("lists candidate runs in newest-first order", async () => {
    const tempDir = await mkdtemp(path.join(tmpdir(), "ouroboros-s18-candidate-run-ledger-"));
    try {
      ...
      const result = await runNode([
        "scripts/trading-candidate-run-ledger.mjs",
        "--root",
        tempDir,
        "--limit",
        "2"
      ...
      expect(result.code, scriptOutput(result)).toBe(0);
      expect(result.stdout).toContain("Trading candidate run ledger");
  ...
  it("registers the npm candidate run commands", async () => {
    const packageJson = JSON.parse(await readFile(path.join(repoRoot, "package.json"), "utf8"));

    expect(packageJson.scripts["trading:candidate:run"])
      .toBe("tsx apps/runtime/src/trading-candidate/run-candidate.ts");
    expect(packageJson.scripts["trading:candidate:run:ledger"])
      .toBe("node scripts/trading-candidate-run-ledger.mjs");
  });
});
...
async function writeCandidateBundle(
  ...
  await writeFile(path.join(artifactDir, "manifest.json"), `${JSON.stringify({
    id: "candidate-run-test-artifact",
    name: "Candidate run test artifact",
...
await event({ event: "order_intent", ...intent });
