...
import { runCodexAarProposalDryRunAudit } from "../src/aar-orchestration/codex-aar-proposal-dry-run-audit";
import { codexAarProposalDryRunFixtureIds } from "../src/aar-orchestration/codex-aar-proposal-dry-run";
...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), "ouroboros-codex-aar-audit-"));
});
...
describe("Codex AAR proposal dry-run audit", () => {
...
`,
    ...
    const audit = await runCodexAarProposalDryRunAudit({
      store_root: tmpDir,
      working_directory: tmpDir,
      codex_command: fakeCodex,
      idempotency_key: "codex-aar-audit-subprocess-success",
    ...
    expect(audit.delta).toEqual({
      ...
      aar_artifact_proposals: 1,
      runnable_artifacts: 1,
      aar_artifact_lineages: 1,
      aar_orchestration_runs: 1
    });
  ...
  it("proves the local success path without requiring real Codex in unit tests", async () => {
    ...
    const calls: string[][] = [];
    const providerAdapter = new CodexCliAarProposalProviderAdapter({
    ...
    const audit = await runCodexAarProposalDryRunAudit({
      ...
      idempotency_key: "codex-aar-audit-success",
  ...
  it("records unavailable Codex without partial proposal, artifact, lineage, or run writes", async () => {
    ...
    const audit = await runCodexAarProposalDryRunAudit({
      store_root: tmpDir,
      provider_adapter: providerAdapter,
      idempotency_key: "codex-aar-audit-missing",
  ...
  it("records invalid provider output without partial proposal, artifact, lineage, or run writes", async () => {
    ...
    const audit = await runCodexAarProposalDryRunAudit({
      store_root: tmpDir,
      provider_adapter: providerAdapter,
      idempotency_key: "codex-aar-audit-invalid",
...
function providerOutput(): AarProposalProviderOutput {
  return {
    ...
    proposal_summary: "Codex dry-run audit proposes the next opaque BTC perp artifact input.",
    ...
    proposed_artifact_refs: [ref("codex_cli_aar_proposal_output", "codex-aar-audit-output")],
