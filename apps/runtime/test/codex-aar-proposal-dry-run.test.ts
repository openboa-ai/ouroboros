...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), "ouroboros-codex-aar-dry-run-"));
});
...
describe("Codex AAR proposal dry-run command runner", () => {
  it("runs Codex CLI through the adapter and materializes proposal records", async () => {
    ...
    const outcome = await runCodexAarProposalDryRun({
      ...
      idempotency_key: "codex-aar-dry-run-command-success",
  ...
  it("records failed provider output without proposal records when Codex is unavailable", async () => {
    ...
    const outcome = await runCodexAarProposalDryRun({
      store_root: tmpDir,
      provider_adapter: providerAdapter,
      idempotency_key: "codex-aar-dry-run-command-missing",
    ...
    await expect(store.listAarArtifactLineages()).resolves.toMatchObject([
      {
...
function providerOutput(): AarProposalProviderOutput {
  return {
    ...
    proposal_summary: "Codex dry-run proposes the next opaque BTC perp artifact input.",
    ...
    proposed_artifact_refs: [ref("codex_cli_aar_proposal_output", "codex-aar-dry-run-output")],
