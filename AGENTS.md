...
| `PaperTradingComparisonCheckpointAttempt` | Append-only first-checkpoint intent persisted before either side refreshes sandbox evidence; it binds the owned `both_running` attempt, exact first tick, side evaluation/observation baselines, provider-request baselines, and deadline. |
| `PaperTradingComparisonCheckpointOutcome` | Append-only terminal first-checkpoint result. `paired` means one atomic LocalStore bundle committed both side Ledger/Observation/Evaluation evidence against the same first tick; `incomplete` means no economic bundle was committed. |
| `PaperTradingComparisonCheckpointWriteContext` | Exact internal authority for one side's sandbox-evidence refresh or the paired commit boundary. It grants no ordinary Ledger, observation, evaluation, lifecycle, private, or live write authority. |
