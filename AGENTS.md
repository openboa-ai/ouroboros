...
| `PaperTradingComparisonConfirmationCampaignOutcome` | Append-only external aggregate decision over every reserved campaign slot. Only all improved slots produce protocol-level `eligible`; the outcome remains sealed, paper-only, `not_live`, and cannot create TradingPromotion or release research memory. |
