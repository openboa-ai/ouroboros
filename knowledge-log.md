...
- Split the desktop app composition so `App` is only the boot/controller entrypoint while a dedicated dashboard screen owns the live-ready layout and inspector composition.
...
- Extracted workspace normalization, checkpoint materialization, restore/import activation bookkeeping, and export-bundle generation into `src-tauri/src/workspace/lifecycle.rs`, shrinking `workspace.rs` into a coordinator-oriented module instead of a monolithic read/write/materialize file.
...
- Added explicit workspace policy modules for import preflight, export-bundle metadata, and live-lane state transitions so repository methods now lean more heavily on `transport -> application -> repository -> policy/transition` boundaries instead of mixing rule evaluation with file-IO orchestration.
- Replaced ad hoc string flags for live/import/workspace status with shared typed runtime-state contracts covering automation status, managed-agent orchestrator mode, import preflight state/severity, and operation status across Rust models, workspace files, and the TypeScript service contract.
