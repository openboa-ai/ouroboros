...
const defaultExecFileRunner: ExecFileRunner = async (file, args, options = {}) => {
  ...
  const gate = sessionToken
    ? await createProviderProcessGate({ sessionToken })
    : undefined;
  return new Promise((resolve, reject) => {
    const processGroup = process.platform !== "win32";
    const gatedCommand = gate?.command(file, args) ?? { file, args };
    ...
    child.on("error", (error) => {
      if (gate) {
        void gate.cleanup().finally(() => failBeforeSpawn(error));
        return;
      }
