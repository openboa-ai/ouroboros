...
def commands(identifier, config):
    ...
    if identifier == 'config.startup':
        ...
        return [([python, str(ROOT / 'tests/contracts/test-config-startup.py'), '--bin-dir', configured(config, 'bin_dir')], 180)]
    if identifier in ('core.transactions', 'core.mutations', 'resources.receipts', 'ingress.api_cli', 'resources.api', 'conversations.api', 'wake.api'):
        ...
        return [([python, str(ROOT / 'tests/support/run-postgres-suite.py'), '--source-root', str(ROOT),
    ...
    if identifier.startswith('native.'):
        ...
        return [([python, str(ROOT / 'tests/support/run-native-suite.py'), '--environment',
    ...
    if identifier.startswith('recovery.'):
        ...
        cmd = [python, str(ROOT / 'tests/recovery' / script)]
...
def require_build(config):
    from tests.support.check_build import validate_build
...
def main():
    ...
    if args.command == 'build':
        from tests.support.check_build import build
    ...
    if args.command == 'package':
        from tests.support.check_build import package, validate_build
        current = source_digest()
        config = environment(args.environment)
