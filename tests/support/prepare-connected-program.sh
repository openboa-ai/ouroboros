...
source_digest=$(python3 "$repo_dir/tests/support/native_image_identity.py" --source-root "$repo_dir")
...
rustc --edition=2024 --target aarch64-unknown-linux-gnu -C target-feature=+crt-static \
  -C opt-level=s -C panic=abort "$repo_dir/tests/fixtures/native-network-probe.rs" \
...
USER 65532:65532
DOCKER
[[ $(python3 "$repo_dir/tests/support/native_image_identity.py" --source-root "$repo_dir") == "$source_digest" ]] || \
