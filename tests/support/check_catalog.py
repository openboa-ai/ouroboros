...
_DOCUMENTS = frozenset({
    "tests/README.md", "README.md", "AGENTS.md", "ARCHITECTURE.md", "CORE_DOCTRINE.md", "SOVEREIGN.md",
    "WHITEPAPER.md", "PRODUCT_SPECIFICATION.md", "SECURITY.md", "CODEOWNERS", "LICENSE",
    ".github/pull_request_template.md",
})
_SHARED = frozenset({
    "scripts/check.py", "scripts/ci_environment.py", "scripts/ci_report.py", "tests/__init__.py",
    "Cargo.toml", "Cargo.lock", "rust-toolchain", "rust-toolchain.toml", ".gitignore",
    "tests/support/check.py", "tests/support/check_catalog.py", "tests/tooling/test-check-catalog.py",
    "tests/support/native_scenarios.py", "tests/tooling/test-native-suite-contract.py",
    "tests/integration/test-connected-native-guest.py",
    "tests/support/fixture_config.py", "tests/support/fixture_release.py", "tests/support/test-profile.sh",
    "tests/support/prepare-test-db.py", "tests/support/prepare-resource-dbs.py",
    "tests/support/provision-test-guest.sh", "tests/support/install-test-rust.sh",
})
_SHARED_PREFIXES = (
...
_SCRIPT_RESPONSIBILITIES = {
    "tests/contracts/test-api-cli.py": ("core", "gateway", "cli"),
    "tests/support/prepare-api-fixture.py": ("core", "gateway", "cli"),
    "tests/contracts/test-resource-api.py": ("core", "gateway", "resources", "cli"),
    "tests/contracts/test-conversation-api.py": ("conversations",),
    "tests/contracts/test-wake-api.py": ("wake",),
    "tests/contracts/test-control-mutations.py": ("core",),
    "tests/integration/test-runtime-guest.py": ("runtime",),
    ...
    "tests/support/program_service_host.py": ("runtime", "resources", "adapters", "core", "gateway"),
    ...
    "tests/recovery/test-recovery-archive.py": ("recovery",),
    "tests/recovery/test-recovery-postgres.py": ("recovery",),
    "tests/support/fixture_recovery_schema.py": ("recovery", "core", "resources"),
    "tests/support/fixture_recovery_inspection.py": ("recovery", "core", "gateway"),
    "tests/fixtures/storage_fault_proxy.py": ("resources",),
}
_TOOLING_SCRIPTS = frozenset({
    "tests/tooling/test-build-profile.py", "tests/tooling/test-fixture-config.py",
    "tests/tooling/test-fixture-release.py", "tests/contracts/test-config-startup.py",
})
...
def _affected(path: str) -> tuple[str, ...] | None:
    ...
    if (path in _SHARED or path.startswith(_SHARED_PREFIXES)
            or path.startswith("tests/support/native") or path.startswith("tests/support/fixture_native")
            or path.startswith("tests/support/prepare-connected") or path.startswith("tests/support/prepare-codex")
            or path.startswith("tests/support/prepare-checkpoint")):
        return None
