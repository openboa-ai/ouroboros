...
write('process-provider.json',{'listen':endpoint('provider'),'tls':tls('provider'),'core_url':url('core'),'core_client':tls('provider'),'gateway_fingerprint':fps['gateway-service'],'role':'provider','database_url_file':str(root/'process-consumer.url'),'key_file':str(root/'process-key.bin'),'provider_ca_file':str(root/'cert.pem'),'provider':binding,'provider_managed_versions':True})
