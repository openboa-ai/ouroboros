...
from tests.support.check_catalog import build_plan
...
def load_native_driver():
    path = Path(__file__).resolve().parents[2] / 'tests/support/run-native-suite.py'
...
class CheckRunnerContract(unittest.TestCase):
    ...
    def test_failed_command_stops_its_scenario_and_retains_current_identity(self):
        plan = self.plan(('tests/contracts/test-config-startup.py',))
    ...
    def test_local_and_full_plan_have_explicit_base_identity(self):
        for full in (False, True):
            with self.subTest(full=full):
                ...
                arguments = ['tests.support.check.py', 'plan', '--output', str(output)] + (['--full'] if full else [])
                def git(*args):
                    if args[0] == 'rev-parse':
                        return '2' * 40 + '\n'
                    if args[0] == 'diff':
    ...
    def test_explicit_base_head_still_includes_dirty_and_untracked_source(self):
        arguments = ['tests.support.check.py', 'plan', '--base', 'HEAD', '--output', str(self.output)]
        def git(*args):
            if args[0] == 'rev-parse':
                return '2' * 40 + '\n'
            if args[0] == 'diff':
    ...
    def test_noncurrent_head_is_rejected_before_source_attestation(self):
        arguments = ['tests.support.check.py', 'plan', '--head', 'previous', '--output', str(self.output)]
        def git(*args):
            if args[0] == 'rev-parse':
