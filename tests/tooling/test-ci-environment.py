...
import tests.support.ci_environment as ci
from tests.support.ci_report import render, traceback_locations

kernel_spec = importlib.util.spec_from_file_location('kernel_builder', Path(__file__).resolve().parents[2] / 'tests/support/prepare-kernel-contract.py')
...
class PublicDiagnostics(unittest.TestCase):
    def test_traceback_exports_only_known_test_file_and_line(self):
        raw = 'File "/private/canary/tests/support/native_adapter_fixture.py", line 108\nsecret-canary\nFile "/private/tests/support/secret-canary.py", line 9'
        self.assertEqual(traceback_locations(raw), ['support/native_adapter_fixture.py:108'])
        report, _ = render({}, [{'id': 'native.bounded-worker', 'status': 'FAIL',
            'failure_locations': ['support/native_adapter_fixture.py:108', '/private/canary', 'secret-canary.py:9']}])
        self.assertIn('support/native_adapter_fixture.py:108', report)
