...
Example (all paths are supplied by the selected host's configuration):
  python3 scripts/run-native-suite.py --environment /absolute/native.json \
...
from fixture_release import hash_file, install
from native_build_input import hash_build_binary
from native_image_identity import CLI_LABEL, MATERIALIZER_LABEL, PROBE_LABEL
from native_kernel_contract import TEST_NAME, verify_manifest as verify_kernel_manifest
from native_scenarios import SCENARIOS, catalogue, select_scenario
from native_suite_environment import CODEX_VERSION, child_environment, docker, load_environment, preflight
...
class NativeRun:
    ...
    def prepare(self):
        ...
        for pattern in ('Cargo.toml', 'Cargo.lock', 'crates/**/*.rs', 'crates/**/*.sql',
                        'crates/**/Cargo.toml', 'scripts/*.py', 'scripts/*.sh', 'scripts/fixtures/*.rs'):
    ...
    def execute(self):
        ...
        self.command([sys.executable, '-B', self.environment['source_root'] / 'scripts' / driver,
        ...
        if self.scenario.driver == 'installation':
            ...
            self.command([sys.executable, '-B', self.environment['source_root'] / 'scripts/test-service-install.py',
        ...
        if self.scenario.restart:
            command = [sys.executable, '-B', self.environment['source_root'] / 'scripts/test-existing-storage-guest.py',
