...
from postgres_suite_support import (Cancellation, Commands, PostgreSQL, SuiteFailure, cargo_result,
...
def management_fixture(args, root, commands, pg, suite):
    ...
    commands.run(suite + "-database-prepare", [sys.executable, str(scripts / "prepare-test-db.py"), "--config", str(db_config)])
    ...
    commands.run(suite + "-fixture-prepare", [sys.executable, str(scripts / "prepare-api-fixture.py"), "--config", str(config)])
...
def api_suite(args, root, commands, pg, suite):
    ...
    scripts = args.source_root / "scripts"
    commands.run(suite, [sys.executable, str(scripts / ("test-" + suite + ".py")), "--config", str(config)],
                 timeout=args.timeout_seconds)
...
def resource_api_suite(args, root, commands, pg):
    ...
    commands.run("resource-api", [sys.executable, str(args.source_root / "scripts/test-resource-api.py"),
...
def mutation_suite(args, root, commands, pg):
    ...
    target.mkdir(mode=0o700)
    commands.run("control-mutations", [sys.executable, str(args.source_root / "scripts/test-control-mutations.py"),
