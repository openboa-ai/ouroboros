...
const DEFAULT_ROOT = path.join(".ouroboros", "trading-system-replay-runs");
const DEFAULT_LIMIT = 10;

if (process.argv.includes("--help") || process.argv.includes("-h")) {
  console.log(`Usage: npm run trading:replay:ledger
       npm run trading:replay:ledger -- --limit 5
       npm run trading:replay:ledger -- --root <path> --json

Lists local TradingSystemCandidate replay runs without running agents, providers, sandboxes, or audits.

Options:
  --root <path>       Replay run root. Default: ${DEFAULT_ROOT}
...
try {
  ...
  if (error?.code === "ENOENT") {
    ...
    console.error(`ERROR failed to read trading replay run ledger: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
...
console.log("Trading replay run ledger");
