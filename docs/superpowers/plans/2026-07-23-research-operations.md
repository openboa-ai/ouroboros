...
- Final promotion review then found two projection-display defects and four remote CodeQL source alerts. Source commit `2ed872e57544722876231216106fe37e6bad266e` (tree `ce9b95fb5e81e2a5da0dbae57dd4cc0ea7bd1034`) closes them: terminal checkpoint/tick selection now reports an unavailable selected artifact when its Evaluation or commitment graph is missing while true `no_submission` remains `not_selected`; completed experiment progress prefers exact checkpoint count, then exact terminal-tick count, then the legacy one-Evaluation fallback; the Web test no longer performs incomplete markup stripping; LocalStore safe-ID edge trimming uses bounded scans; and two unused projection helpers were removed. The four focused suites passed 135/135, application/LocalStore/Operator Web typechecks passed, `git diff --check` passed, and independent selected-artifact and experiment-count reviews returned PASS. Fresh remote CodeQL and all current-head PR checks remain mandatory before merge.
