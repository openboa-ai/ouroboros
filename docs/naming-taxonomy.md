...
| `PaperTradingComparisonCheckpointAttempt` | Append-only first-checkpoint intent persisted before side effects. It binds the exact owned `both_running` attempt, first tick, side baselines, provider-request baselines, and deadline. |
| `PaperTradingComparisonCheckpointOutcome` | Append-only terminal first-checkpoint result. `paired` proves one atomic bundle committed both sides against the same first tick; `incomplete` proves the attempt closed without an economic bundle. |
| `PaperTradingComparisonCheckpointWriteContext` | Internal authority scoped only to exact sandbox-evidence refresh or paired checkpoint commit. It cannot authorize ordinary lifecycle, Ledger, observation, evaluation, private, or live writes. |
