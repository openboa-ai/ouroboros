...
| `CandidateArenaResearchAllocation` | Append-only pre-effect research-only scheduling decision for one CandidateArena tick; it freezes the exact goal/time/event/recovery trigger, selected and deferred directions, bounded experiment budgets and concurrency, signal provenance, and an explicit-request, repository-default, or exact approved-decision policy basis without becoming economic or promotion evidence. |
| `ResearchPreflightCommitment` | Append-only pre-effect binding of one tick, direction, worker, allocation, source SystemCode, hypothesis, method, exact ResearchEvidenceArtifact refs/digests, bounded development policy, and evaluator-owned rotating sealed-admission suite. It stores only commitments/digests, permits one sealed submission, and grants no admission, promotion, order, private, or live authority. |
...
| `RuntimeSupervisor` | Store-scoped runtime coordinator that reconciles selected PaperTrading sessions, persisted CandidateArena start intent, and the ResearchControlStudyScheduler in fixed order with bounded progress-aware retries and reverse-order shutdown. It has runtime-coordination authority only. |
| `RuntimeSupervisorCheckpoint` | Immutable predecessor-linked record of RuntimeSupervisor lane desire, status, basis/progress digests, attempts, retry time, and reason. It reconstructs coordination after restart and is not evaluation or promotion evidence. |
| `RuntimeProcessOwnership` | Same-host operational identity and immutable transition history for one RuntimeSupervisor, provider, or deterministic long-running Sandbox process. It binds host, PID start marker, executable/profile, runtime-supervisor, worker/store-root, or Sandbox process scope, runtime, and session token before effects; only an exact live Sandbox owner may be adopted, while a provider owner is terminated before a fresh session. It has no research, evaluation, order, private, or live authority. |
...
| `RuntimeSoakHarness` | External operational test controller that runs or resumes one immutable time-bounded fault schedule, samples normalized runtime state, and terminalizes on exact invariant, target, or duration evidence. It is not a RuntimeSupervisor lane and grants no product authority. |
| `RuntimeSoakReport` | Create-only manifest and predecessor-linked event chain for one RuntimeSoakHarness run. It preserves elapsed time, action intent/completion, normalized samples, first failure, and terminal classification with operational-test authority only. |
...
| `ResearchEvidenceArtifact` | Immutable sanitized Research input with exact source and artifact digests. Version 1 accepts Arena paper result, Arena trace, Arena failure, or released Finding; future live evidence requires a separate provenance validator. It carries no hidden qualification, credential, private-data, promotion, order, or live authority. |
...
| `OperatorReadModel` | Shared operator state returned to CLI, UI, and TUI, including the same RuntimeSupervisor projection exposed by runtime health. |
