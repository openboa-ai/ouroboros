...
| Research generalization | Shows prospective protocol lifecycle, condition-block progress, deadline, latest conservative cross-study outcome, and latest approval-only research-policy decision. | `CandidateArenaReadModel.research_generalization`. | Read-only `not_promotion_authority`; no mutation or ResearchWorker feedback. The underlying decision may authorize only future uncontrolled allocation provenance. | Keep before Finding clusters and Research signals. | Preserve simultaneous active/outcome/decision evidence, show explicit false downstream authorities, and exclude raw studies, artifacts, digests, and per-slot effects. |
...
| Is prospective cross-study generalization collecting, awaiting reconciliation, closed, or consumed by a research-policy decision? | `CandidateArenaReadModel.research_generalization`. | CLI status, TUI Arena scan, Web Research. | Pure projection graph tests, operator HTTP integration, interface parity, and active/outcome/decision/empty surface tests. | Real-market protocol completion and generated or tuned policy parameters remain separate frontiers. |
...
- `CandidateArenaReadModel.research_generalization` carries required compact prospective protocol
  lifecycle, outcome, and latest policy-decision evidence. CLI, TUI, and Web Research render the
  same status, active progress, latest inference, decision status/effective mode, next action, and
  `not_promotion_authority`; Web also renders exact source IDs and explicit false downstream
  authorities. The projection is not researcher context and does not itself alter allocation.
