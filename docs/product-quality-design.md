...
| `FindingCluster` | Group paper-backed findings into reusable next-generation research pressure. | CandidateArena read model, researcher context, and Research tab read-only surface. | Clusters must teach generation without becoming rank, qualification, Trading review blockers, direction scheduling, or promotion authority. | Later adaptive direction selection may read clusters only through an explicit policy frontier with separate tests. |
...
| What should the next ResearchWorker learn? | CandidateArena read model/context, findings, lineage, paper-board learning, and finding clusters. | Research tab and researcher prompt context. | CandidateArena read-model/context tests plus Web Research surface tests. | Adaptive direction selection from those clusters remains future work. |
...
| `packages/application/src/candidate/arena.ts` | Strong keep. CandidateArena context now learns from paper evidence instead of replay score alone. | Research context carries paper-board learning, selected paper evidence lineage, latest research efficiency, and finding clusters. | Adaptive direction selection may read these signals only through a future explicit policy frontier; clusters must stay out of rank, qualification, review readiness, and promotion. |
...
| P2 | Keep finding clusters as shared read-model and Research context. | Research should improve from accumulated evidence, not isolated anecdotes. | `CandidateArenaReadModel`, researcher context, and Web Research surface. | CandidateArena read-model/context tests plus Web Research tests showing direction, blocker, market regime, protocol failure, next focus, and `not_promotion_authority`. | Clusters guide generation only; no rank, qualification, Trading review blocker, direction scheduling, or promotion authority. |
...
`not_promotion_authority` and only guides the next ResearchWorker. Web Research renders the same
cluster as read-only next-generation context and labels its `ResearchWorker input` plus `Cluster
boundary`: no rank, no qualification, no Trading review blocker, no direction scheduling, and no
promotion. CandidateArena read-model/context tests and Web surface tests prove a paper protocol
failure enters the next tick state and operator Research surface without changing paper rank,
qualification, Trading review, direction scheduling, or promotion authority.
