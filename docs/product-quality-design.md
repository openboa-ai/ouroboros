...
| What should the next ResearchWorker learn? | CandidateArena read model/context, findings, lineage, paper-board learning, finding clusters, and adaptive direction focus. | Research tab and researcher prompt context. | CandidateArena read-model/context tests plus Web Research surface tests. | Budget-aware direction selection from provider cost/latency remains future work. |
