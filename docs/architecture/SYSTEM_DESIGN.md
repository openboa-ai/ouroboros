...
| Credentials and fixed Responses worker | Existing authenticated encryption, owner/version binding, enrollment/disable receipts and fixed protected sender | Independent Auth Module loading/selection, OAuth refresh, general HMAC/mTLS, external-key use and authenticated incoming events remain unimplemented/unqualified; do not relabel bounded verification as operating acceptance |
